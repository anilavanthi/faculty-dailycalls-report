...
def dashboard():
    ...
    if request.method == "POST":
        ...
        if student:
            conn = get_db()
            c = conn.cursor()
            c.execute('''INSERT INTO calls (user_id, student, phone_number, status, notes, call_date, exam_type)
                         VALUES (?, ?, ?, ?, ?, ?, ?)''',
                      (session['user_id'], student, phone_number, status, notes, call_date, exam_type))
            conn.commit()
            conn.close()

    conn = get_db()
    c = conn.cursor()
    c.execute('''SELECT u.username, c.student, c.phone_number, c.status, c.notes, c.call_date, c.exam_type
                 FROM calls c JOIN users u ON c.user_id = u.id
                 WHERE u.id=? 
                 AND (c.exam_type = "EAPCET" OR c.exam_type = "POLYCET" OR c.exam_type = "General")  -- This is the update
...
def export_excel():
    ...
    df = pd.DataFrame(data, columns=["Faculty", "Student", "Phone Number", "Status", "Notes", "Date", "Exam Type"])
...
def export_my_excel():
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute('''SELECT student, phone_number, status, notes, call_date, exam_type
                 FROM calls WHERE user_id=? ORDER BY call_date DESC''', (session['user_id'],))
    data = c.fetchall()
    conn.close()

    df = pd.DataFrame(data, columns=["Student", "Phone Number", "Status", "Notes", "Date", "Exam Type"])
